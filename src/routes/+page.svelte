<script>
	...
	function addchat(thing) {
		if (!(thing == null || thing == '')) {
			const eventRef = collection(db, 'things');
			addDoc(eventRef, { name: thing, time: serverTimestamp(), uid: userID }, { merge: true });
...
{:else if signIn}
	<div
		class="flex flex-col items-center min-h-screen max-h-full justify-center bg-white border-solid"
	>
		<div class="">
				...
				<div class=" mt-3 flex flex-col border-b items-stretch w-full">
					{#each foods.reverse() as food}
						{#if food.data().uid == userID}
							<div
								class="flex flex-row justify-between bg-blue-500 rounded-xl p-2 w-full my-1 text-white shadow-cyan-950"
							>
								<p class="font-semibold">{food.data().name}</p>
								{#if userID == '1TZEvWyy85ge1b2kOyUT4zZQlOq1'}
									<button on:click={deletefoo(food)} class="font-semibold">delete</button>
								{/if}
							</div>
						{:else}
							<div
								class="flex flex-row justify-between bg-white 0 rounded-xl p-2 w-full my-1 shadow-lg text-blue-900"
							>
								<p class="font-semibold">{food.data().name}</p>
								{#if userID == '1TZEvWyy85ge1b2kOyUT4zZQlOq1'}
									<button on:click={deletefoo(food)} class="font-semibold">delete</button>
